...
public class WordCram {
	...
	public enum TextCase { Lower, Upper, Keep };
	public TextCase textCase = TextCase.Keep;
...
//	}
	...
	public WordCram withAlgorithm(int a){
		this.algorithm = a;
		return this;
	}
	...
	public String getExtraStopWords(){
		return this.extraStopWords;
	}
	...
	public void setConfiguration(int cloudIndex, long rSeed, int scale, boolean force) {
...
//		System.err.println();
//		System.err.println("cloud Index "+cloudIndex);
//		for(int ii = 0; ii< 50; ++ii){
//			System.err.println(words[ii].word+" "+words[ii].weight+" "+words[ii].origWeight);
//		}
		if (fonter == null) fonter = Fonters.alwaysUse(parent.createFont("serif", 1));
		if (sizer == null) sizer = Sizers.byWeight(5, 70);
		if (colorer == null) colorer = Colorers.alwaysUse(parent.color(0));
		if (angler == null) angler = Anglers.mostlyHoriz(rSeed);
		if (placer == null) placer = Placers.horizLine(rSeed);
		if (nudger == null) nudger = new SpiralWordNudger();
		
		words = new WordSorterAndScaler().sortAndScale(words,false);
		if (renderOptions.maxNumberOfWordsToDraw >= 0 && renderOptions.maxNumberOfWordsToDraw < words.length) {
			words = Arrays.copyOf(words, renderOptions.maxNumberOfWordsToDraw);
		}
		words = new WordSorterAndScaler().normalizeWordsL1(words);
		if(scale == 1) words = new WordSorterAndScaler().scaleByBoundingBox(words,fonter,sizer);
		for(Word w : words){
			w.setCloudIndex(cloudIndex);
//			System.out.println(w);
//			if(force && false) w.weight = 0.5f; // QUIM
		}
	}
	
	public WordCramEngine getWordCramEngine() {
		if (wordCramEngine == null) {
			int leafSize = (algorithm == 3 || algorithm == 4)? -1 : 1;
			PGraphics canvas = destination == null? parent.g : destination; 
			wordCramEngine = new WordCramEngine(canvas, words, fonter, sizer, colorer,
					angler, placer, nudger, new WordShaper(), new BBTreeBuilder(leafSize), renderOptions);
		}
	...
	public void moveNext(){
		getWordCramEngine().moveNext();
	}
	
	public HashMap<String,ArrayList<String>> moveAll(){
		return getWordCramEngine().moveAll();
	}
	
	public void placeNext() {
		getWordCramEngine().placeNext();
	}
	
	public boolean placeAll() {
		return getWordCramEngine().placeAll();
	}
	
	public void placeUnique(Map<String, CoordProp> index, Word[] unique, long rSeed) {
		getWordCramEngine().placeUnique(index,unique, rSeed);
	}
	
	public void show(Map<String, CoordProp> index, int cloudInd){
		getWordCramEngine().show(index, cloudInd);
	}
	...
	public Word[] getUniqueWords() {
		Word[] wordsCopy = new Word[unique.length];
		System.arraycopy(unique, 0, wordsCopy, 0, unique.length);
		return wordsCopy;
	}
	
	public void setWords(Word[] wordsCopy){
		if(words == null){
			words = new Word[wordsCopy.length];
		}
		if(wordsCopy.length != words.length){
			words = new Word[wordsCopy.length];
		}
		System.arraycopy(wordsCopy, 0, words, 0, wordsCopy.length);
	}
	
	public void setUniqueWords(Word[] wordsCopy){
		if(unique == null){
			unique = new Word[wordsCopy.length];
		}
		if(wordsCopy.length != unique.length){
			unique = new Word[wordsCopy.length];
		}
		System.arraycopy(wordsCopy, 0, unique, 0, wordsCopy.length);
	}
	...
	void reduceWordSize(float factor){
		for(Word w: words){	
			w.weight = w.weight*factor;
			w.refactorSize(factor);
			w.notSkipped();		
		}
		for(Word w: unique){	
			w.weight = w.weight*factor;
