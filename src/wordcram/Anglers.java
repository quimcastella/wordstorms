...
public class Anglers {
	 ...
	 */
	public static WordAngler random(long rSeed) {
	 ...
	 *         time it's called.
	 */
	public static WordAngler randomBetween(long rSeed, final float min, final float max) {
		final Random r = new Random(rSeed);
	 ...
	 */
	public static WordAngler heaped(long rSeed) {
		final float angle = PApplet.radians(7);
		return randomBetween(rSeed, -angle, angle);
	}
	 ...
	 *         each word.
	 */
	public static WordAngler pickFrom(long rSeed, final float... angles) {
		final Random r = new Random(rSeed);
		return new WordAngler() {
			public float angleFor(Word w) {
	 ...
	 */
	public static WordAngler hexes(long rSeed) {
		float oneSixth = PConstants.TWO_PI / 6f;
		return pickFrom(rSeed, 0f, oneSixth, 2 * oneSixth, 3 * oneSixth, 4 * oneSixth,
				5 * oneSixth);
	}
	 ...
	 *         and down.
	 */
	public static WordAngler upAndDown(long rSeed) {
		return pickFrom(rSeed, PConstants.HALF_PI, -PConstants.HALF_PI);
	}
	 ...
	 *         rest vertically.
	 */
	public static WordAngler mostlyHoriz(long rSeed) {
		return pickFrom(rSeed, 0f, 0f, 0f, 0f, 0f, PConstants.HALF_PI, -PConstants.HALF_PI);
	}
}
